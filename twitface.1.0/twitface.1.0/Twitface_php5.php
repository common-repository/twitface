...
function Twitface_rest_client($secret, $session_key) {
	return new FacebookRestClient(Twitface_FB_APIKEY, $secret,
		$session_key);
}

function Twitface_fbclient_setfbml_impl($fbclient, $text) {
	try {
...
function Twitface_fbclient_publishaction_impl($fbclient, $template_id,
		$template_data,
...
function Twitface_fbclient_getinfo($fbclient, $fields) {
	try {
...
function Twitface_fbclient_getsession($fbclient, $token) {
	try {
